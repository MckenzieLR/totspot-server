...
from totspotapi.models import Parent, Provider
    ...
    if account_type is not None \
        ...
        if account_type == 'parent':
            ...
            if phone_number is None:
                return Response(
                    {'message': 'You must provide a phone number for a parent'},
            ...
            if phone_number is None:
                return Response(
                    {'message': 'You must provide a phone number for an employee'},
                    status=status.HTTP_400_BAD_REQUEST
                )
        else:
            return Response(
                {'message': 'Invalid account type. Valid values are \'parent\' or \'provider\''},
                status=status.HTTP_400_BAD_REQUEST
            )
